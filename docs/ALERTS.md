...
The synthetic path also applies a **headway-scaled silence gate** (`SYNTHETIC_HEADWAY_MULT = 3` in `bin/train/pulse.js`): even though the base 20-min lookback may be empty for the line, before synthesizing the bin re-queries observations over `max(LOOKBACK_MS, 3 × headway)`. If any obs exist in the longer window, the candidate is dropped. Rationale: on single-train shuttle topologies (Yellow), a normal terminus turnaround layover already runs ~15–17 min, so a single missed/delayed turnaround can drop the line to zero obs in the 20-min window despite only ~1 trip being missed. The 3× multiplier comfortably clears the natural turnaround variance while still admitting the multi-hour shutdown case the synthetic path was built for (shuttle-bus substitution etc.). Lines with peak headway ≤ ~6 min (Red/Blue/Brown peak) hit the 20-min floor and see no change; Yellow @ 15-min headway now requires ≥ 45 min of full-line silence. Added 2026-05-12 after a 33-min Yellow turnaround gap synthesized a false-positive blackout.
