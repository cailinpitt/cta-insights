...
| Command | Description |
|---|---|
| `node bin/export-web.js [output-path]` | Read `state/history.sqlite` (readonly) and emit `alerts.json` with `schema_version: 2` and unified **`incidents[]`** — each pairing an official CTA/Metra alert (`official_alert`) with bot detections (`detections[]`) from `disruption_events` and `roundup_anchors`. `buildIncidents` runs the official-alert↔detection merge here (so the web frontend doesn't have to), normalizes train line keys to full names, and emits grouped `lifecycle`, `scope`, and Metra `status` blocks. Writes to `output-path` if given, otherwise stdout. Safe to run alongside live cron jobs. Format changes are recorded in the web repo's [data changelog](https://chicagotransitalerts.app/data/CHANGELOG.md). |
| `bin/push-web-data.sh` | Wrapper for the cron-driven publish loop. Exports `alerts.json`, `daily-counts.json`, and `alerts.csv` into `tmp/web-data`, uploads changed files to the R2 data origin (`https://data.chicagotransitalerts.app`), and fires a GitHub `repository_dispatch` rebuild so the static site can refresh prerendered pages, feeds, CSV, sitemap, and share cards. Set `CTA_INSIGHTS`, `RCLONE_REMOTE`, `DISPATCH_REPO`, and `GITHUB_DISPATCH_TOKEN` when the defaults do not match production. Run every 15 minutes by cron; posting jobs may also trigger it event-driven. |
